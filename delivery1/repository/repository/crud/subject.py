...
from repository.models.subject import Subject, SubjectCreate, PublicKeyCreate
...
class CRUDSubject(CRUDBase[Subject, SubjectCreate, str]):
    ...
    async def create(self, create_obj: SubjectCreate) -> Subject:
        subject = Subject.model_validate(create_obj)
        subject = await self._add_to_db(subject)
        await crud_public_key.create(
            PublicKeyCreate(
